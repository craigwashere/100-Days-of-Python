...
# Create a client using the credentials and region defined in the test_user
# section of the AWS credentials and configuration files
session = Session(profile_name="test_user")
...
class ChunkedHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def route_read(self, path, query):
        ...
        if len(text) == 0 or len(voiceId) == 0 or outputFormat not in AUDIO_FORMATS:
            ...
            raise HTTPStatusError(HTTP_STATUS["BAD_REQUEST"], "Wrong parametersxcvxcv")
        else:
            try:
    ...
    def send_headers(self, status, content_type):
        ...
        self.send_header('Access-Control-Allow-Origin', 'https://mozilla.github.io/pdf.js/build/pdf.mjs')
        self.end_headers()
